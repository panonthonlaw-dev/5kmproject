...
# --- 4. ส่วนหัวโปรแกรม (Top-Left Login Area) ---
# สร้าง 4 คอลัมน์ โดยใช้คอลัมน์แรกสุดเป็นที่วางระบบล็อกอิน
...
if not st.session_state.logged_in:
    ...
    except: st.info("กำลังโหลด Leaderboard...")

else:
    # --- หน้า Admin (โหมดบันทึกคะแนน) ---
    st.markdown("### 🎯 บันทึกคะแนนรายบุคคล")
    ...
    with st.container(border=True):
        ...
        if st.button("🚀 ยืนยันบันทึกคะแนน", use_container_width=True, disabled=not can_save):
            try:
                # 1. บันทึกเจาะจงช่องเดียว (Surgical Update) ไม่ยุ่งส่วนอื่น
    ...
    # แสดงประวัติการบันทึกล่าสุด 5 รายการ (ตัด Leaderboard ออกเพื่อความเร็ว)
    if not logs_df.empty:
        st.markdown("---")
