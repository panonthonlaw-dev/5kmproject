...
def resurrection_event(players_list):
    ...
    st.write("รายชื่อเพื่อนที่กำลังหลับใหล (คะแนนน้อยกว่า 10)")
    ...
    if inactive.empty:
        st.balloons()
        st.success("สุดยอด! ทุกคนในระบบแข็งแกร่งหมดแล้ว")
    else:
        # 2. 🟢 ส่วนที่ทำให้เลื่อนได้ (Scrollable Area)
        # กำหนดความสูงไว้ที่ 300 (ถ้าเกินนี้จะเลื่อนได้เอง)
        with st.container(height=300):
            for _, row in inactive.iterrows():
                # บีบระยะบรรทัดให้ชิดกันสไตล์ Super Compact
                c1, c2 = st.columns([3, 1])
                c1.markdown(f"👤 {row['Name']}")
                c2.markdown(f"EXP `{row['EXP']}`")
        
        st.divider()
        st.info("📢 **กติกา:** ชวนเพื่อนในรายชื่อกลับมาเล่น เมื่อเพื่อนมีแต้มเพิ่ม จะบวกแต้มพิเศษให้คนชวนด้วย!")
