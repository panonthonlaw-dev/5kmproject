...
# --- 1. Page Config & CSS (ล็อก 5 คอลัมน์สมดุล) ---
...
st.markdown("""
    ...
    html, body { font-family: 'Sarabun', sans-serif; overflow-x: hidden; background-color: #fcfcfc; }
    ...
    .data-label { font-size: 1.9vw !important; color: #888; }
    .data-val { 
        ...
        if f_list:
            ...
            if st.button("🚀 บันทึกคะแนน", use_container_width=True):
                try:
                    ...
                    # แก้ปัญหา NaN และคำนวณคะแนนใหม่
                    raw_v = pd.to_numeric(f_df.at[idx, s_day], errors='coerce')
                    f_df.at[idx, s_day] = int((0 if pd.isna(raw_v) else raw_v) + a_pts)
                    
                    # --- [THE FIX] ส่งกลับเฉพาะคอลัมน์ A ถึง AK เท่านั้น ---
                    # iloc[:, :37] คือเอาคอลัมน์ index 0 ถึง 36 (A ถึง AK)
                    # จะไม่ส่งคอลัมน์ 37 (AL) เป็นต้นไปกลับไปทับสูตร
                    update_df = f_df.iloc[:, :37]
                    ...
                    new_log = pd.DataFrame([{
                        ...
                        "Student": sel_n, "Day": s_day, "Points": a_pts, "Status": "Success"
                    }])
                    conn.update(worksheet="Logs", data=pd.concat([log_df, new_log], ignore_index=True))
                    
                    st.success("บันทึกสำเร็จ! ข้อมูลใน Sheets ปลอดภัย 100%")
                    st.cache_data.clear()
                    st.rerun()
