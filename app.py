...
# CSS: ล็อก 5 คอลัมน์สมบูรณ์แบบ และปรับแต่ง UI
st.markdown("""
    <style>
    ...
    @media (min-width: 1024px) {
        .player-card { padding: 15px; min-height: 180px; }
...
def get_sh():
    # ใช้ gspread สำหรับการบันทึกแบบเจาะจงพิกัด (Surgical Update)
...
def load_data():
    # ใช้ GSheetsConnection สำหรับการอ่านข้อมูล (เร็วและมี Cache)
...
if not st.session_state.logged_in:
    ...
    # --- โหมดแอดมิน: บันทึกคะแนน + ระบบค้นหา + ล็อกบันทึกซ้ำ ---
    ...
    with st.container(border=True):
        ...
        if not logs_df.empty:
            ...
            # เช็กจากคอลัมน์ Student(C) และ Day(E) ของวันนี้
        ...
        if st.button("🚀 ยืนยันบันทึกคะแนน", use_container_width=True, disabled=not can_save):
            try:
                # 1. บันทึกเจาะจงช่องเดียว (Surgical Update) เพื่อรักษาความปลอดภัยของสูตร
                main_ws = sh.worksheet("Sheet1")
                r_idx = main_ws.find(sel_name, in_column=1).row
                c_idx = main_ws.find(sel_day, in_row=1).col
                old_val = main_ws.cell(r_idx, c_idx).value
                main_ws.update_cell(r_idx, c_idx, int(float(old_val or 0)) + pts)
                
                # 2. บันทึกลง Logs (ลำดับ A:Time | B:Admin | C:Student | D:Points | E:Day)
                log_ws.append_row([
    ...
    # แสดงประวัติการบันทึกย่อๆ แทน Leaderboard เพื่อความเร็ว
    if not logs_df.empty:
        st.markdown("---")
