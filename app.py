...
def resurrection_event(players_list):
    st.markdown("### ✨ **ภารกิจ: ปลุกพลังเพื่อน**")
    st.write("📢 **กติกา:** ชวนเพื่อนในรายชื่อกลับมาเล่น เมื่อเพื่อนมีแต้มเพิ่ม คุณครูจะบวกแต้มพิเศษให้คนชวนด้วย!")
    
    # 1. เตรียมข้อมูล
    df = pd.DataFrame(players_list)
    inactive = df[df['EXP'] <= 10][['Name', 'EXP']].sort_values('EXP')
    
    if inactive.empty:
        st.balloons()
        st.success("สุดยอด! ทุกคนแข็งแกร่งหมดแล้ว")
    else:
        # 2. 🟢 ส่วนที่เลื่อนได้ และบังคับให้อยู่บรรทัดเดียวกัน
        with st.container(height=350):
            for _, row in inactive.iterrows():
                # ใช้ Flexbox บังคับให้อยู่ซ้าย-ขวา ในบรรทัดเดียว
                st.markdown(f"""
                <div style="display: flex; justify-content: space-between; align-items: center; 
                            border-bottom: 0.5px solid #eee; padding: 8px 2px; gap: 10px;">
                    <div style="font-size: 0.85rem; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                        👤 {row['Name']}
                    </div>
                    <div style="background: #e3f2fd; color: #1e88e5; padding: 2px 8px; 
                                border-radius: 10px; font-size: 0.8rem; font-weight: bold; min-width: 60px; text-align: center;">
                        ⭐ {row['EXP']} EXP
                    </div>
                </div>
                """, unsafe_allow_html=True)
        ...
        st.divider()
        st.info("เมื่อเพื่อนกลับมาทำกิจกรรมแล้วให้แจ้งcare teamเพื่อรับคะแนนพิเศษ")
